...
{
    [TestFixture]
    public class ProcessConnectedTest
    {
        [Test]
        public void Connected_ohne_vorherigen_start()
        {
            IPtClientStarter ptClientStarter = new PtClientStarter();

            try
            {
                ptClientStarter.ProcessConnectedMessage(new ConnectedMessage());
                Assert.Fail("Connected kann ohne vorherigen Start ist nicht gestattet");
            }
            catch (InvalidOperationException)
            {
                // Alles ok - Exception sollte ohne vorherige StartClient-Nachricht auftreten
            }
        }

        [Test]
        public void Connected_doppelt_senden()
        {
            IPtClientStarter ptClientStarter = new PtClientStarter();
            ...
            {
                ptClientStarter.ProcessConnectedMessage(new ConnectedMessage());
                Assert.Fail("ConnectedMessage darf nicht zweimal verarbeitet werden!");
            }
            catch (InvalidOperationException)
            {
